    ...
    else
    {
      if ($opcaoscanner == "0")
        ...
        elseif ($opcaoscanner == "1") {
          ...
          // Lista de pacotes de gerenciadores de arquivos e compactadores
          $gerenciadoresArquivos = [
              ...
              'com.android.documentsui',            // Documentos Android
              'com.sec.android.app.myfiles',        // Samsung My Files
              'com.miui.fileexplorer',              // MIUI File Explorer
              ...
              'com.mobisystems.fileman',            // OfficeSuite File Manager
              ...
              'com.filemanager.free',               // File Manager Free
              ...
              'com.xiaomi.mifilesexplorer'          // Mi Files Explorer
          ];

          // Verificar últimos acessos aos gerenciadores de arquivos
          echo $bold . $amarelo . "[*] Verificando acessos recentes a gerenciadores de arquivos...\n";
          ...
          foreach ($gerenciadoresArquivos as $pacote) {
              ...
              if (!empty($resultadoVerificarInstalado)) {
                  // App está instalado, verificar último acesso
                  ...
                  if (!empty($resultadoUltimoAcesso)) {
                      ...
                      if (!empty($matches[1])) {
                          $timestampUltimoAcesso = $matches[1] / 1000; // Converter de milissegundos para segundos
                          $dataUltimoAcesso = date("d-m-Y H:i:s", $timestampUltimoAcesso);
                          
                          // Calcular há quanto tempo foi o último acesso
                          $diferencaHoras = round((time() - $timestampUltimoAcesso) / 3600, 1);
                          
                          // Mostrar nome amigável dos apps mais conhecidos
                          $nomeAmigavel = "";
                          ...
                          else $nomeAmigavel = basename($pacote);
                          
                          if ($diferencaHoras < 2) {
                              echo $bold . $vermelho . "[!] " . strtoupper($nomeAmigavel) . " ACESSADO: " . $dataUltimoAcesso . " (há " . $diferencaHoras . " horas)\n";
                              $appAcessados[] = ['nome' => $nomeAmigavel, 'data' => $dataUltimoAcesso];
                          } else {
          ...
          // Verificar desinstalações recentes de QUALQUER aplicativo
          echo "\n" . $bold . $amarelo . "[*] Verificando desinstalações recentes (últimas 2 horas)...\n";

          // Recuperar timestamp atual e de 2 horas atrás
          $tempoAtual = time();
          $tempoLimite = $tempoAtual - (2 * 3600); // 2 horas atrás
          $tempoLimiteMs = $tempoLimite * 1000; // Converter para milissegundos

          // Verificar data de última desinstalação no sistema
          ...
          if (!empty($resultadoUltimaDesinstalacao)) {
              ...
              if (!empty($matches[1])) {
                  ...
                  if ($timestampDesinstalacao > $tempoLimiteMs) {
          ...
          $comandoPacotesRemovidos = 'adb shell "dumpsys package | grep -A10 \'Recently removed packages\' | grep -v \'Last removal time\'"';
          ...
          if (!empty($resultadoPacotesRemovidos) && strpos($resultadoPacotesRemovidos, "Empty") === false) {
              // Extrair os pacotes da saída
              preg_match_all('/Recently removed packages:(.+?)(?=\n\w|\z)/s', $resultadoPacotesRemovidos, $matches);
              
              if (!empty($matches[1][0])) {
                  $pacotesLista = trim($matches[1][0]);
                  $pacotes = preg_split('/\s+/', $pacotesLista);
                  
                  $pacotesFiltrados = array_filter($pacotes, function($pacote) {
                      return !empty(trim($pacote)) && $pacote != "Empty";
                  });
                  
                  if (!empty($pacotesFiltrados)) {
                      echo $bold . $vermelho . "[!] APPS DESINSTALADOS RECENTEMENTE:\n";
                      
                      foreach ($pacotesFiltrados as $pacote) {
                          echo $bold . $vermelho . "    - DESINSTALADO: " . $pacote . "\n";
                      }
                  }
              }
          }

          // Verificar desinstalações via logs do sistema
          $comandoLogsDesinstalacao = 'adb shell "logcat -d -b main -v time | grep -i \'Package removed\'"';
          $resultadoLogsDesinstalacao = trim(shell_exec($comandoLogsDesinstalacao));

          if (!empty($resultadoLogsDesinstalacao)) {
              $linhas = explode("\n", $resultadoLogsDesinstalacao);
              $desinstalacoesLogsRecentes = [];
              
              foreach ($linhas as $linha) {
                  if (preg_match('/(\d{2}-\d{2} \d{2}:\d{2}:\d{2}).*Package removed: ([^\s]+)/', $linha, $matches)) {
                      if (!empty($matches[1]) && !empty($matches[2])) {
                          $dataDesinstalacao = $matches[1];
                          $pacoteDesinstalado = $matches[2];
                          
                          $date = DateTime::createFromFormat('m-d H:i:s', $dataDesinstalacao);
                          if ($date) {
                              $timestampDesinstalacao = $date->getTimestamp();
                              if ($timestampDesinstalacao > $tempoLimite) {
                                  $formattedDate = $date->format('d-m H:i:s');
                                  $desinstalacoesLogsRecentes[] = ['pacote' => $pacoteDesinstalado, 'data' => $formattedDate];
                              }
                          }
                      }
                  }
              ...
              if (!empty($desinstalacoesLogsRecentes)) {
                  echo $bold . $vermelho . "[!] DESINSTALAÇÕES NAS ÚLTIMAS 2 HORAS (LOGS DO SISTEMA):\n";
                  foreach ($desinstalacoesLogsRecentes as $desinstalacao) {
                      echo $bold . $vermelho . "    - " . $desinstalacao['pacote'] . " DESINSTALADO: " . $desinstalacao['data'] . "\n";
                  }
                  $desinstalacoesRecentes = true;
              }
          }
          ...
          echo "\n"; "estrongs") !== false) $nomeAmigavel = "ES FILE EXPLORER";
